...
class DES_kab
{
    ...
    function hitung()
    {
        ...
        for ($a = 1; $a <= $this->n_periode; $a++) {
            $key = 7;
            $this->ft_next[$key] = $this->last_at + $this->last_bt * $a;
            $this->selisih[$key] = $this->ft_next[$key] - $k_temp;
                 if ($this->selisih[$k] > 0) {
                    if($this->q == 1) {
                        $this->ft_next_p[$key] = $this->ft_next[$key] + $this->selisih[$key];
                    }
                    
                    if($this->q == 2) {
                        $this->ft_next_p[$key] = $this->ft_next[$key] + (0.75 * $this->selisih[$key]);
                    }

                    if($this->q == 3) {
                        $this->ft_next_p[$key] = $this->ft_next[$key] + (0.5 * $this->selisih[$key]);
                    }

                    if($this->q == 4) {
                        $this->ft_next_p[$key] = $this->ft_next[$key] + (0.25 * $this->selisih[$key]);
                    }
                 }
